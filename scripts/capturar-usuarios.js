...
async function run() {
  const dir = path.resolve(__dirname, '../manual-screenshots/usuarios');
  ...
  console.log('¿OTP visible?:', hasOtp);
  if (hasOtp) {
    ...
    console.log('Texto actual tras enviar credenciales:', (await page.locator('body').textContent()).slice(0, 300));
    await page.screenshot({ path: path.join(dir, 'login-debug.png') });
  }

  console.log('2. Esperando sistema post-login. URL:', page.url());
  await page.waitForTimeout(3000);

  // Navegar a /catalogos/usuarios
  console.log('3. Navegando a Catálogos > Usuarios...');
  try {
    const catBtn = page.getByText(/Catálogos/i).first();
    if (await catBtn.isVisible({ timeout: 4000 }).catch(() => false)) {
      await catBtn.click();
      await page.waitForTimeout(1000);
      const uBtn = page.getByText(/^Usuarios$/i).first();
      if (await uBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
        await uBtn.click();
      } else {
        await page.goto('https://erpfoly.vercel.app/catalogos/usuarios');
      }
    } else {
      await page.goto('https://erpfoly.vercel.app/catalogos/usuarios');
    }
  ...
  console.log('URL actual en Usuarios:', page.url());

  // Captura 1: Listado General
  await page.screenshot({ path: path.join(dir, '01-listado-usuarios.png') });
  ...
  const tablaData = await page.evaluate(() => {
    ...
    return { title, ths, filas: filas.slice(0, 5) };
  });
  console.log('DATOS TABLA USUARIOS:', JSON.stringify(tablaData, null, 2));
  ...
  // Captura 4: Formulario Nuevo Usuario (/catalogos/usuarios/nuevo)
  const btnNuevo = page.getByRole('button', { name: /nuevo usuario|^nuevo$/i }).or(page.getByText(/Nuevo usuario/i)).first();
  if (await btnNuevo.isVisible()) {
    await btnNuevo.click();
    await page.waitForTimeout(2000);
    await page.screenshot({ path: path.join(dir, '03-formulario-nuevo-usuario.png') });
    console.log('Captura 3 lista (Formulario Nuevo Usuario)');
  ...
  if (await actionButton.isVisible()) {
    ...
    console.log('Captura 4 lista (Menú de Acciones)');

    // Extraer opciones del menú
    const menuItems = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('[role="menuitem"], .MuiMenuItem-root, li')).map(el => el.textContent?.trim()).filter(Boolean);
    });
    console.log('OPCIONES MENU USUARIOS:', JSON.stringify(menuItems));
    ...
    if (await editarOpt.isVisible()) {
      ...
      await page.waitForTimeout(2000);
      await page.screenshot({ path: path.join(dir, '05-formulario-editar-usuario.png') });
      console.log('Captura 5 lista (Formulario Editar Usuario)');
      await page.goto('https://erpfoly.vercel.app/catalogos/usuarios');
      await page.waitForTimeout(3000);
    }
  }

  // Captura 6: Modal / Confirmación Desactivar
  ...
  if (await actionButton2.isVisible()) {
    ...
    if (await desactivarOpt.isVisible()) {
      ...
      await page.screenshot({ path: path.join(dir, '06-dialogo-desactivar-usuario.png') });
      console.log('Captura 6 lista (Diálogo Desactivar)');
      await page.keyboard.press('Escape');
      await page.waitForTimeout(1000);
    }
  }

  console.log('¡Todas las capturas de Usuarios completadas exitosamente!');
  await browser.close();
}

run().catch(err => {
  console.error('Error al capturar Usuarios:', err);
  process.exit(1);
});
