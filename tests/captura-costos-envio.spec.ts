...
test.describe('Manual Exhaustivo - Catálogos > Costos de envío', () => {
  test.use({ 
    storageState: 'playwright/.auth/user.json',
    viewport: { width: 1440, height: 900 }
  });

  test('Capturar todas las vistas, modales y acciones de Costos de envío', async ({ page }) => {
    ...
    // Expandir Catálogos y hacer clic en Costo de envío
    await page.getByText(/^Catálogos$/i).first().click();
    await page.waitForTimeout(1000);

    const subitemCosto = page.locator('nav, aside, [role="navigation"]').getByText(/costo.*env[ií]o/i).first();
    await subitemCosto.click();
    await page.waitForTimeout(3500);

    console.log('URL actual:', page.url());

    // Captura 1: Listado General de Costos de Envío
    await page.screenshot({ path: path.join(dir, '01-listado-costos-envio.png') });
    ...
    console.log('DATOS CATALOGO COSTOS ENVIO:', JSON.stringify(tablaData, null, 2));

    // Captura 2: Búsqueda dinámica
    const searchInput = page.getByPlaceholder(/buscar/i).first();
    if (await searchInput.isVisible()) {
      await searchInput.fill('Zona');
      await page.waitForTimeout(1000);
    ...
    // Captura 4: Modal / Formulario Nuevo Costo de Envío
    const btnNuevo = page.getByRole('button', { name: /^Nuevo$/i }).or(page.getByText(/^Nuevo costo/i)).first();
    if (await btnNuevo.isVisible()) {
      await btnNuevo.click();
      await page.waitForTimeout(2000);
      await page.screenshot({ path: path.join(dir, '04-modal-nuevo-costo-envio.png') });
      console.log('Captura 4 lista (Nuevo Costo)');
      ...
      console.log('CAMPOS MODAL NUEVO COSTO:', JSON.stringify(modalData, null, 2));
    ...
    if (await menuBtn.isVisible()) {
      ...
      console.log('OPCIONES MENU COSTOS:', opciones);
      ...
      if (await editarOpt.isVisible()) {
        ...
        await page.waitForTimeout(2000);
        await page.screenshot({ path: path.join(dir, '06-modal-editar-costo-envio.png') });
        await page.keyboard.press('Escape');
        await page.waitForTimeout(1000);
      }
      ...
      if (await desactivarOpt.isVisible()) {
        ...
        await page.screenshot({ path: path.join(dir, '07-dialogo-desactivar-costo.png') });
        await page.keyboard.press('Escape');
        await page.waitForTimeout(1000);
      }
    }

    console.log('¡Catálogo de Costos de Envío capturado al 100%!');
  });
});
