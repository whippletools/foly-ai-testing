...
test.describe('Manual Exhaustivo - Catálogos > Usuarios', () => {
  test.use({ 
    storageState: 'playwright/.auth/user.json',
    viewport: { width: 1440, height: 900 }
  });

  test('Capturar todas las vistas, modales y acciones de Usuarios', async ({ page }) => {
    const dir = path.resolve('manual-screenshots/usuarios');
    ...
    // Expandir Catálogos y hacer clic en Usuarios
    await page.getByText(/^Catálogos$/i).first().click();
    await page.waitForTimeout(1000);

    const subitemUsuarios = page.locator('nav, aside, [role="navigation"]').getByText(/^Usuarios$/i).first();
    await subitemUsuarios.click();
    await page.waitForTimeout(3500);

    console.log('URL actual:', page.url());

    // Captura 1: Listado General de Usuarios
    ...
    console.log('DATOS CATALOGO USUARIOS:', JSON.stringify(tablaData, null, 2));
    ...
    // Captura 4: Modal / Formulario Nuevo Usuario
    const btnNuevo = page.getByRole('button', { name: /^Nuevo$/i }).or(page.getByText(/^Nuevo usuario/i)).first();
    if (await btnNuevo.isVisible()) {
      await btnNuevo.click();
      await page.waitForTimeout(2000);
      await page.screenshot({ path: path.join(dir, '04-modal-nuevo-usuario.png') });
      console.log('Captura 4 lista (Nuevo Usuario)');
      ...
      console.log('CAMPOS MODAL NUEVO USUARIO:', JSON.stringify(modalData, null, 2));
    ...
    if (await menuBtn.isVisible()) {
      ...
      console.log('OPCIONES MENU USUARIOS:', opciones);
      ...
      if (await editarOpt.isVisible()) {
        ...
        await page.waitForTimeout(2000);
        await page.screenshot({ path: path.join(dir, '06-modal-editar-usuario.png') });
        await page.keyboard.press('Escape');
        await page.waitForTimeout(1000);
      }
      ...
      if (await desactivarOpt.isVisible()) {
        ...
        await page.screenshot({ path: path.join(dir, '07-dialogo-desactivar-usuario.png') });
        await page.keyboard.press('Escape');
        await page.waitForTimeout(1000);
      }
    }

    console.log('¡Catálogo de Usuarios capturado al 100%!');
  });
});
